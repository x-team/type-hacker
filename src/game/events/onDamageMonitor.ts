...
export const onDamageMonitor = ({
  ...
  scene
    .getPlayerData()
    .data.monitors[monitorToBeDamaged].guessText?.setText("");
  scene.getPlayerData().data.monitors[monitorToBeDamaged].userText?.setText("");
  getNextWord({
    scene,
    guessWord:
      scene.getPlayerData().data.monitors[monitorToBeDamaged].guessText,
  ...
  setTimeout(() => {
    scene.scene
      ...
      .events.emit("show-clock", monitorToBeDamaged);
    scene.scene
      .get(SceneKeys.Panels)
      .events.emit("reset-clock", monitorToBeDamaged);
  }, getDelayBetweenWords(scene.getPlayerData()));

  scene.getPlayerData().data.monitors[monitorToBeDamaged].isDamaged = true;
  scene.scene
    .get(SceneKeys.Smoke)
    .events.emit("create-smoke", monitorToBeDamaged);
};
